...
const PasswordRecoveryForm = ({ onSubmit, isLoading, error, success }: PasswordRecoveryFormProps) => {
  ...
  // If recovery email was sent successfully, show success message
  if (success) {
    return (
    ...
    <div className="border rounded-lg p-6 bg-card shadow-sm max-w-md w-full mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <Button type="submit" disabled={isLoading} className="mt-2 relative">
          ...
          <span className={isLoading ? "opacity-0" : ""}>{isLoading ? "Sending..." : "Reset Password"}</span>
        </Button>
      </form>
